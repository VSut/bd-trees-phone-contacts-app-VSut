...
public class ContactDao {
    ...
    public SortedMap<Name, Contact> getContacts(SortBy sortBy, SortOrder sortOrder) {
        ...
        if (sortBy == SortBy.FIRST_NAME) {
            baseMap = new TreeMap<>(this.contactsSortedByFirstName);
        } else {
            baseMap = new TreeMap<>(this.contactsSortedByLastName);
        }
        if (sortOrder == SortOrder.DESCENDING) {
            baseMap = new TreeMap<>(baseMap).descendingMap();
        } else {
            baseMap = new TreeMap<>(baseMap);
        }
        return baseMap;
    }
